/*
* Action 및 Mutationdml Methed 목록 상수 정의
*/
export default {
  SET_IS_LOADING: 'setIsLoading',
  FETCH_CREDENTIAL: 'fetchCredential',
  FETCH_CUSTOM_MODELS: 'fetchCustomModels',
  CREATE_CUSTOM_MODEL: 'createCustomModel',
  DELETE_CUSTOM_MODEL: 'deleteCustomModel',
  SET_VIDEO_PLAYER_OBJECT: 'setVideoPlayerObject',
  SET_VIDEO_PLAYER_CURRENT_TIME: 'setVideoPlayerCurrentTime',
  SET_VIDEO_PLAYER_LOOP: 'setVideoPlayerLoop',
  FETCH_CUSTOM_MODEL: 'fetchCustomModel',
  SET_STATE_LOGIN: 'setStateLogin',
  SET_STATE_VIDEOFILE: 'setStateVideofile',
  RECOGNIZE_VIDEO: 'recognizeVideo',
  SET_FILENAME: 'setFileName',
  SET_CORPUS_NAME: 'setCorpusName',
  SPLICE_RECOGNIZE_RESULT: 'spliceRecognizeResult',
  SET_RECOGNIZE_RESULT: 'setRecognizeResult',
  SET_SUBTITLES: 'setSubtitles',
  SET_SUBTITLE: 'setSubtitle',
  SET_PROGRESS_STATUS: 'setProgressBar',
  SPLICE_SUBTITLE: 'spliceSubtitle',
  EXPORT_XML: 'exportXml',
  FETCH_CORPUS: 'fetchCorpus',
  TRAIN_MODEL: 'trainCustomModel',
  SET_SELECTED_MODEL_NAME: 'setSelectedModelName',
};
